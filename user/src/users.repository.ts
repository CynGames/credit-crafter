...
async getById(user_id: string): Promise<UserDTO>{
    const queryText = 'select u.first_name, u.last_name, u.email, u.address1, u.phone_number, u.created_at, u.updated_at, f.credit_score, f.income, f.expenses\
    from users u \
...
async getByEmail(email: string){
    const queryText = 'select u.user_id, u.first_name, u.last_name, u.email, u.address1, u.phone_number, u.created_at, u.updated_at, f.credit_score, f.income, f.expenses\
    from users u \
    ...
    try{
        ...
        const row = result.rows[0];
        const user = new UserDTO(row.user_id, row.first_name, row.last_name, 
         row.address1, row.email, row.phone_number, row.created_at, 
         row.updated_at, row.credit_score, row.income, row.expenses);
