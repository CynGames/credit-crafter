      ...
      eachMessage: async ({ topic, partition, message }) => {
        ...
        switch (topic) {
          ...
          case LOAN_UPDATE_REQUEST:
            try {
              await this.loanService.changLoanState(
                parsedMessage.payload.loanId,
                parsedMessage.payload.state,
              );
          ...
          case PAYMENT_CREATE_REQUEST:
            try {
              const payment_id = await this.loanService.createPayment(
          ...
          case PAYMENT_FETCH_REQUEST:
            try {
              const payments = await this.loanService.getPaymentsByLoan(
                parsedMessage.payload.loan_id.loan_id,
              );
